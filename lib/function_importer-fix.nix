  ...
  in {
    options = {
      ...
      builderOptionsSchema = lib.mkOption {
        ...
        # Move imports INSIDE the submodule definition
        type = lib.types.submodule {
          imports = [
            defaultOptionDeclsFile
            (if builtins.pathExists scopedOptionDeclsFile then scopedOptionDeclsFile else {})
          ];
        };
      };
      ...
      builderOutputModule = lib.mkOption {
        ...
        default = importApply functionFile {
          inherit pkgs;
          # Passes the evaluated config from the submodule to the builder
    ...
    modules = [
      {
        ...
        # Populate the builders attribute with empty configs to trigger default evaluation
